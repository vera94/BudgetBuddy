...
function addItem(event) {
	...
	$.ajax({
		url : "http://localhost:8080/BudgetBuddy/items/"+ tableid + "/add",
		type : "POST",
		contentType : "application/json",
		data : JSON.stringify(data)
	}).success(function(data) {
		$("#addItem").attr("action", "http://localhost:8080/BudgetBuddy/view/PersonalTable.html");
	}).fail(function(data) {
		//$("#register_form").attr("action", "http://localhost:8080/BudgetBuddy/view/PersonalTable.html");
	}).always(function() {
		$("#addItem").submit();
	});
}
...
$(document).ready(function() {
	...
	function renderRow(rowData, table) {
		...
		row.append(detailsTd);
		 ...
		 link.click(function() {
		 $.ajax({
		 url: "http://localhost:8080/BudgetBuddy/items/" + tableId + "/delete/"+ rowData.id,
		 type: "DELETE",
		 success: alert("Item deleted successfuly")
		 });
		 });
